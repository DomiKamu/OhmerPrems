...
- COMING SOON: **Entirely free KlokSpid MkII**, the successor of *KlokSpid* (2017 / Ohmer Modules), is a 16HP "CPU-controlled" clocking module, 8 analog/digital outputs. It embeds 6 configurable CV sources (upto 8 by changing "CLK" and "RUN" input roles, may become bipolar -5V/+5V or unipolar 0/+10V CV source - "CLK" input can be assigned as "CV1" input, "RUN" input as "CV2" input, and native "CV3" as "RESET" input - useful for internal phase reset purposes). Basically, KlokSpid MkII is able to output pulses/gates (min. 1ms duration, pulse width can be set and possibly modulated by any CV source), triangles (including ramps up/down by changing and modulating tilt), sine (first half sine is fixed, but second half can be amplitude-morphed to become redressed sine, until hump), S&H (using internal random generator, variable density - min. is one per "BPM x ratio" tick), and 8 customizable wavetables (manually morphable, possibly by CV) by this way, KlokSpid MkII can act as envelope generator! Each output can be phase-shifted, quantized, and played or not (every BPM x ratio tick) thanks to euclidean rhythm settings. As master clock, BPM range is 1 upto 900 (15Hz). As slave clock (behind external clock source), the max. frequency is 500Hz (above BPM 900, the external clock speed is indicated as frequency in Hz. Frequency is displayed as red color if over 500Hz). All LEDs (all inputs, all outputs, and RUN/STOP button) are RGB - their color follow this convention: RED for out-of-range CV input voltage, disabled output (aka output's modifier is set to OFF - useful for CPU save), and during tempo "SYNC" operation (RUN/STOP LED, regardling external clock source)... CYAN for pulse-based input ("CLK" tick, "RUN" as held gate or RUN/STOP pulse switch, and possibly CV3 if reassigned as "RESET" pulse-based input), and utility voltage applied to output (GATE, START, or STOP event) when applicable... Variable GREEN for valid CV voltage on input, voltage sent to output (even when stopped if voltage offset is not 0V)... YELLOW on "CLK" input - slave, only when set as "BPM CV" (fully compatible with [Impromptu Modular CLOCKED](https://library.vcvrack.com/ImpromptuModular/Clocked) **BPM CV** feature)... For each output, most defined parameters (modifier ratio picked from built-in table, pulse width/tilt/deform/density/morph, voltage amplitude, voltage offset, phase shift, quantization offset, and euclidean steps/pulses/offset/pads) can be modulated by CV (only waveform shape setting can't be modulated). Any output (all settings) can be duplicated to another output. Possibility to reset a particular output settings to defaults(factory). Via a specific option from context menu, output 8 is able to output the module's phase (aka master phase) - **this option, when enabled, takes priority over output 8 regular settings** - as **unipolar 0V/+10V sawtooth**, this feature may be useful to sync other modules on KlokSpid MkII tempo, debugging purposes, and so on...
