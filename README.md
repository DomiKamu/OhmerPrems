...
- **6OP-DX Kompakt** will be the compact player-like version of 6OP-DX, significantly thinner (34HP), but without any DX7 edit features (except **MONOPHONIC** state toggle). Current selected voice settings can be viewed (access from MENU button), but can't be changed (except MONOPHONIC state, and modulation matrix). Like 6OP-DX, this compact "player" will be fully compatible with DX7 SysEx files (import only), also **.6opsynth** entire synthesizer file format, and **6pocart** 32-voice cartridge (both with extended features, generated by the _6OP-DX_ module only). Its [**User's Manual**](docs/modules/6OP-DX-Kompakt/Manual.md) is under construction, too!
