...
- COMING SOON: **Entirely free KlokSpid MkII** - the successor of (2017) *KlokSpid* - a 16HP CPU-controlled clocking module, 8 analog (wavetable-based) outputs, 6 configurable CV modulators (upto 12 CV inputs by using KX expander, one or two more by changing "CLK" and/or "RUN" input roles). KlokSpid MkII will output 1ms triggers, ratchets, gates, envelopes / LFO (1-bar length), S&H/random, audio, into -5V/5V range...
