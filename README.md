...
- **6OP-DX Kompakt** will be the compact "free player" version of 6OP-DX, significantly thinner (34HP), but without any DX7 edit features (except **MONOPHONIC** toggle, and modulations matrix). Current selected voice settings can be viewed (access from MENU button), but can't be changed (except MONOPHONIC state, and modulation matrix). Like 6OP-DX, this compact "player" will be fully compatible with DX7 SysEx files (import only), and **.6opdx** file format (32-voice cartridge with extended features, generated by the _6OP-DX_ module only). Its [**User's Manual**](docs/modules/6OP-DX-Kompakt/Manual.md) is under construction, too!
