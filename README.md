...
- COMING SOON: **Entirely free KlokSpid MkII** - the successor of (2017) *KlokSpid* - a 16HP CPU-controlled clocking module, 8 analog (wavetable-based) outputs, 6 configurable CV modulators (upto 8 CV inputs by changing "CLK" and/or "RUN" input roles). KlokSpid MkII will output 1ms triggers, ratchets, gates, envelopes / LFO, S&H/random, into -5V/5V range, even in audio range frequencies. All LED (inputs, outputs, RUN/STOP button, SET/Cancel buttons) are RGB.
