...
- **6OP-DX Kompakt** is a **Work In Progress** compact "player-like" version of the _6OP-DX_ module, significantly thinner (34HP, instead of 117HP), but without any DX7 parameter edit features (except **MONOPHONIC** state toggle, and fully editable modulation matrices). Current selected voice DX7 parameters can be viewed only (access from MENU button), but can't be edited. Like _6OP-DX_ module, this compact instrument is fully compatible with DX7 SysEx files (but import only). Proprietary binary **.6opsynth** synthesizer files, and **.6pocart** 32-voice cartridge files (both embeds extended features who are not supported by DX7 SysEx) can be loaded and saved (modulation matrix edits, MONOPHONIC changed states). Its [**User's Manual**](docs/modules/6OP-DX-Kompakt/Manual.md) is under construction, too!
