...
- **6OP-DX Kompakt** is a **Work In Progress** compact "player-like" version of the _6OP-DX_ module, significantly thinner (34HP, instad of 117HP), but without any DX7 parameter edit features (except **MONOPHONIC** state toggle, and fully editable modulation matrices). Current selected voice DX7 parameters can be viewed only (access from MENU button), but can't be edited. Like _6OP-DX_ module, this compact instrument will be fully compatible with DX7 SysEx files (import only), also **.6opsynth** synthesizer file, and **6pocart** 32-voice cartridge file (both .6opsynth and .6opcart files have extended features who don't exist in SysEx). Its [**User's Manual**](docs/modules/6OP-DX-Kompakt/Manual.md) is under construction, too!
