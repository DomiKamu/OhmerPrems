...
- COMING SOON (*TLTR* I'm sorry in advance ;): **Entirely free KlokSpid MkII**, the successor of *KlokSpid* (2017 / [Ohmer plugin](https://github.com/DomiKamu/Ohmer), is a 16HP "CPU-controlled" modern clocking module, 8 analog/digital outputs (right side). It embeds by default 6 configurable CV sources (all inputs are located on left side), but can be increased to 8 by changing "CLK" and "RUN" input roles (both may become bipolar -5V/+5V CV input, or unipolar 0/+10V CV input) - either from *Select Input* menu on the main screen, or via context menu. "CLK" input can be assigned as "CV1" input, and "RUN" input as "CV2" input. Also, native "CV3" input (bipolar -5V/+5V CV, or unipolar 0/+10V CV) as "RESET" input - useful for internal phases reset purposes. Basically, KlokSpid MkII is able to output pulses/gates (min. 1ms duration, *Pulse width* can be set as fixed, and possibly modulated around fixed setting by any CV source you want), triangles (including ramps up/down, by changing & modulating *Tilt*), sine (first half sine is always fixed, but second half sine can be amplitude-deformed to become *redressed sine*, hump, and any intermediate shapes), S&H (using internal random generator, variable *Density* - minimum is 1 "sample" per BPM tick - modifier ratio is ignored), and 8 customizable wavetables (manually *Morph*able, possibly by a CV source) by this way, KlokSpid MkII can act as envelope generator, or something similar! Each output can be *Phase*-shifted (except S&H), quantized, and played/muted (every "BPM x ratio" tick) via euclidean rhythm settings. As master clock, BPM range is 1 upto 900 (15Hz). As slave clock (when "CLK" is patched to an external clock source, and set as "External CLK" input), the maximum supported frequency is 500Hz (above BPM 900, the external clock speed is indicated as **frequency, in Hz** as cyan color, or red color while the frequency is internally limited to 500Hz). All LEDs (inputs, outputs, and RUN/STOP button) are RGB - colorscheme follows this convention: RED for out-of-range CV input voltage, disabled output (aka output's modifier is set to "OFF" - useful for CPU saving for unused outputs), and during tempo "SYNC" operation (RUN/STOP LED, regardling external clock source speed)... CYAN for pulse-based / gate-based input ("CLK" tick, "RUN" as held gate, or "RUN/STOP" pulse toggle, and possibly "CV3" if assigned as "RESET" pulse-based input to reset phases while clock is running), and applied utility voltage to output (these utility modifiers are "GATE", "START", "STOP", and "OFF")... Variable GREEN for valid CV voltage applied on input, voltage sent to output (even when clock is stopped, if voltage *offset* is not 0V)... YELLOW on "CLK" input - slave & when set as "BPM CV" only (fully compatible with [Impromptu Modular CLOCKED](https://library.vcvrack.com/ImpromptuModular/Clocked) **"BPM CV"** feature)... For each output, most defined parameters can be CV-modulated (by single CV): modifier ratio (picked from built-in table, from /16384 to x128), *Pulse width*/*Tilt*/*Deform*/*Density*/*Morph* who morph the waveform, voltage amplitude, voltage offset, quantization offset, *Phase*-shift, and euclidean settings (steps, pulses, offset, and pads). Only base waveform (shape selected from *Waveform* on-screen menu) and *Unipolar*/*Bipolar* output option (from *Amplitude* on-screen setting) cannot be altered by CV. Any output can be copied to another (meaning all ouput settings are duplicated to target output). Possibility to reset settings to defaults (factory) for any output. Via a specific option (from context menu, MISCELLANEOUS section), output 8 is able to output the module's "master" phase - **however, when enabled, this option takes priority over output 8 regular settings!** - as unipolar 0V/+10V sawtooth signal, following main tempo...
...
Please notice the display shows basic waveform choice, it's the first page of 3 (pages 2 and 3 are for - future - wavetables). All icons below "Pulse", "Triangle", "Sin", and "S&H" labels in clickable boxes, are animated (showing what the morph is doing) ;)
