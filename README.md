...
:warning: _6OP-DX_ is provided as **Free/Trial** for non-OhmerPrems members. By using the free/trial (aka without a valid license V2 keyfile), voice(s), MONOPHONIC states, and modulations matrix are never saved (any change is lost as soon as you close VCV Rack 2). Save and export features (from right click menu) are disabled (related menu entries are grayed).
