...
- COMING SOON: **Entirely free KlokSpid MkII**, the successor of *KlokSpid* (2017 / Ohmer Modules), is a 16HP "CPU-controlled" clocking module, 8 analog/digital outputs, 6 configurable CV modulators (upto 8 by changing "CLK" and "RUN" input roles). "CLK" input may be redefined as "CV1". "RUN" input may be redefined as "CV2". Native "CV3" can be redefined as "RESET" input (phase reset). Basically, KlokSpid MkII outputs +10V 1ms pulses (triggers) by default, but it may output gates, LFO (triangle/sawtooth/ramp, sine) or random (S&H based)... instead, into -5V/+5V range (can be offset upto 0V/+10V, and depending the selected output waveform). Master BPM range is 10 upto 350. As slave, the max. output frequency is 800Hz (controlled by external clock device who are using a high rate factor). All LEDs (inputs, outputs, and RUN/STOP button) are RGB - their color conventions are mainly RED for out-of-range CV input voltages, disabled output, BPM SYNC operation (syncing to external clock source, when slave)... CYAN for regular pulses and gates (CLK/RUN/CV3 inputs, utility voltages on outputs), variable GREEN for valid CV inputs, and active outputs. YELLOW on "CLK" input only when set as "BPM CV" (fully compatible with [Impromptu Modular CLOCKED](https://library.vcvrack.com/ImpromptuModular/Clocked) **BPM CV** feature)...
