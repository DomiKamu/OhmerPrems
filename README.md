...
Also, in case of your current rack doesn't embed any VCV AUDIO module (or present but not configured), **major timing issues** can be noticed, in particular by erratic "blinking" items, such displayed messages or blinking LED.

---

## **WHAT IS OHMERPREMS?**

*OhmerPrems* is a commercial plugin, based on shareware concept, designed for VCV Rack 2. The plugin embeds both **Premium** modules (**full version** modules for customers who have a valid license V2 keyfile, or as "Free/Trial" with limited features, otherwise), and also some free modules for everyone (license V2 keyfile is not required to run them as full version). Source code is not published (closed proprietary source code).
...
- **6OP-DX Kompakt** will be the compact player-like version of 6OP-DX, significantly thinner (34HP), but without any DX7 edit features (except **MONOPHONIC** state toggle). Current selected voice settings can be viewed (access from MENU button), but synthesizer settings can't be changed (except MONOPHONIC state via its dedicated button, and modulation matrix). Like 6OP-DX, this compact "player" will be fully compatible with DX7 SysEx files (import only), also **.6opsynth** whole synthesizer file, and **6pocart** 32-voice cartridge file (both .6opsynth and .6opcart files have extended features, can be created by the _6OP-DX_ module only). Its [**User's Manual**](docs/modules/6OP-DX-Kompakt/Manual.md) is under construction, too!
