...
- COMING SOON: **Entirely free KlokSpid MkII**, the successor of *KlokSpid* (2017 / Ohmer Modules), is a 16HP "CPU-controlled" clocking module, 8 analog/digital output. Six configurable CV sources (upto eight by changing "CLK" and "RUN" input roles, may become bipolar -5V/+5V or unipolar 0/+10V CV source). "CLK" input may be redefined as "CV1", and "RUN" input may be redefined as "CV2" (native "CV3" can be redefined to become a "RESET" input - for internal phases reset purposes). Basically, KlokSpid MkII can output pulses/gates (min. 1ms duration, pulse width can be changed and modulated by any CV source), triangles (including sawtooth and ramp, by changing/modulating tilt), sine (second semi-period can be deformed to become redressed or humps), Sample & Hold (internal random generator, morphable density), and eight customizable wavetables. Each output can be quantized, and played or not (every beat) by euclidean rhythm settings. Master BPM range is 1 upto 900. As slave, the max. output frequency is not yet defined (work in progress). All LEDs (inputs, outputs, and RUN/STOP button) are RGB - their color conventions are mainly RED for out-of-range CV input voltages, disabled outputs (OFF), and tempo SYNCing operation (regardling external clock source, when the module is slave)... CYAN for pulse-based inputs ("CLK", "RUN", and CV3 if set as "RESET") and utility voltages on outputs (GATE, START, STOP events), variable GREEN for valid CV inputs, and active outputs. YELLOW on "CLK" input only when set as "BPM CV" (fully compatible with [Impromptu Modular CLOCKED](https://library.vcvrack.com/ImpromptuModular/Clocked) **BPM CV** feature)...
