...
It's an old (never fixed) VCV Rack issue, since the beginning! The culprit are - in most situations - VCV **AUDIO** (2-channel audio input/ouput module), VCV **AUDIO8**, or/and VCV **AUDIO16** modules installed in your current rack. When they're badly configured, the DSP core is "stucked" (it doesn't start), and potentially may crash VCV Rack 2 (application, or as plugin from DAW). I encourage you to send an email to support@vcv.com to report/confirm this bug. This "stucked DSP" issue affects **all modules in the rack**, any brand, including VCV-branded modules themselves.

This may occur when you open multiple applications who are using same audio interface (sharing the same audio driver **AND** the same audio interface/device, in particular ASIO on Windows platforms). Also, this problem is common when you open a third-party patch (patch made by another user), like downloaded from any website (PatchStorage, VCV Community forum, YouTube), if the AUDIO module settings don't match your audio interface.

Also, in case of your current rack doesn't embed any VCV AUDIO module (or present, but not configured), **major timing issues** can be noticed, in particular by erratic "blinking" items, such displayed messages, lights/LED...
...
**Daily** Pre-release v2.6.14 ALPHA offers **W.I.P./in development** _DeXtral_ & _DeXtral Kompakt_ synth voice modules, for testing purposes only. **It doesn't output sound  at the moment!** (the 96 sound engines are implemented, but disabled, they need optimizations, also a working feedback, and minimalist amp. envelope generators).
...
:information_source: _Vektor_ and _VX_ modules will be **free for everyone**, meaning it will operate as **full version**, whatever you're owner of a license V2 keyfile or not, exactly like KlokSpid MkII & KX, KordZ, and (future) DeXtral / DeXtral Kompakt modules.

---

## **DeXtral: Work In Progress - currently in development!**

![](docs/modules/DeXtral/_img/mainModuleV2.png)

The _DeXtral_ synth voice module is not yet included in the OhmerPrem plugin v2.6.13, but why?
- Some MacBook users continue to suffer about missing panel SVG textures with some combined zoom levels & UI scale factors. This concern all large modules (not only DeXtral), I suspect it's an **issue caused by VCV Rack 2 v2.6.6 graphic engine**. So please be sure to set **UI Scale** (from **View** menu) factor to **Auto** (or **100%**) for more reliable graphical rendering (confirmed by three MacBook users)!
...
However, during development, I'll publish **Pre-release v2.6.14 ALPHA ("Daily" builds)** every day (or often) morning in France (UTC+2). When this module is finished, fine tested, and stable, the plugin version will be v2.6.15 (I assume v2.6.14 as pre-releases ALPHA "Daily" only).
...
:information_source: _DeXtral_ module will be **free for everyone**, meaning it will operate as **full version**, whatever you're owner of a license V2 keyfile or not, exactly like Vektor / VX, KlokSpid MkII & KX, KordZ, and (future) DeXtral Kompakt modules.
...
:information_source: _KlokSpid MkII_ / _KX_ modules will be **free for everyone**, meaning it will operate as **full version**, whatever you're owner of a license V2 keyfile or not, exactly like Vektor / VX, KordZ, and (future) DeXtral / DeXtral Kompakt modules.
...
:information_source: _KordZ_ module will be **free for everyone**, meaning it will operate as **full version**, whatever you're owner of a license V2 keyfile or not, exactly like Vektor / VX, KlokSpid MkII & KX, and (future) DeXtral / DeXtral Kompakt modules.
...
- your email address (preferably used to access to your VCV Library account, or a reliable permanent email address), required to send your personal license key, instructions to install it, and possible future communications about major updates. Your name and email address stays strictly private, and I never spam mailboxes!
- also in the body of the message from PayPal, please indicate **OhmerPrems** (or something similar), to avoid possible confusions!

New subscription requires max. 24 hours for processing, in general less, often couple of hours, sometimes delivered after... minutes, after PayPal payment notification. 

Please consider **incomplete/not regular fee informations or/and payment less than 15 euros equivalent will be refurbished**.
