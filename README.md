...
- **6OP-DX** is a **Work In Progress** 117HP 6-operator 32-algorithm phase modulation (PM) synthesizer voice module (**Panel display issues on MacBook computers vs. custom UI Scale factors - see below, and NO OUTPUT SOUND AT THE MOMENT**). Its [**User's Manual**](docs/modules/6OP-DX/Manual.md) is under construction, too.
- **6OP-DX Kompakt** will be the compact "free player" version of 6OP-DX, significantly thinner (34HP), but without any DX7 edit features (except **MONOPHONIC** toggle). DX7 parameters can be viewed (access from MENU button) but can't be changed. Like 6OP-DX, this compact "player" will be fully compatible with DX7 SysEx files (import only), and .6opdx file format (32-voice cartridge files with extended features, generated by the _6OP-DX_ module). Its [**User's Manual**](docs/modules/6OP-DX-Kompakt/Manual.md) is under construction, too!
...
:warning: _6OP-DX_ is provided as **Free/Trial** for non-OhmerPrems members. By using the free/trial (without valid license V2 keyfile), stuff can't be saved, save and export features are disabled.
