...
- COMING SOON: **Entirely free KlokSpid MkII**, the successor of *KlokSpid* (2017 / Ohmer Modules), is a 16HP "CPU-controlled" clocking module, 8 analog/digital output. Six configurable CV sources (upto eight by changing "CLK" and "RUN" input roles, may become bipolar -5V/+5V or unipolar 0/+10V CV source - "CLK" input may be assigned as "CV1" input, "RUN" input may be assigned as "CV2" input, native "CV3" can be redefined to become a "RESET" input for internal phases reset purposes). Basically, KlokSpid MkII is able to output pulses/gates (min. 1ms duration, pulse width can be altered/modulated by any CV source), triangles (including sawtooth and ramp, by changing/modulating tilt), sine (first half sine is fixed, second half sine can be amplitude-morphed to become redressed sine, until double humps), S&H (using internal random generator, variable density - min. is one per "BPM x ratio" tick), and eight customizable wavetables (manually morphable and/or by any CV). Each output can be phase-shifted, quantized, and played or not (every BPM x ratio tick) by euclidean rhythm settings. Master BPM range is 1 upto 900. **As slave, the max. output frequency is not yet defined (work in progress)**. All LEDs (inputs, outputs, RUN/STOP button) are RGB - their color conventions are mainly RED for out-of-range CV input voltages, disabled outputs (modifier set to OFF), and tempo SYNCing operation (regardling external clock source, when the module is slave)... CYAN for pulse-based inputs ("CLK", "RUN", and CV3 if reassigned as "RESET" input) and utility voltages applied on output (GATE, START, STOP events), variable GREEN for valid CV inputs and active outputs, PINK (concerns output 8 only, when it delivers the module's phase - can be enabled/disabled via context menu). YELLOW on "CLK" input - when slave - only when set as "BPM CV" (fully compatible with [Impromptu Modular CLOCKED](https://library.vcvrack.com/ImpromptuModular/Clocked) **BPM CV** feature)...
