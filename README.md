...
- COMING SOON: **Entirely free KlokSpid MkII** - the successor of (2017) *KlokSpid* - a 16HP CPU-controlled clocking module, 8 analog (wavetable-based) outputs, 6 configurable CV modulators (upto 8 CV inputs by changing "CLK" and/or "RUN" input roles). "CLK" input may be reassigned as "CV1". "RUN" input can be reassigned as "CV2". Native "CV3" can be reassigned (if needed) as "RESET" input. Basically, KlokSpid MkII outputs +10V 1ms triggers, but it may output ratchets, gates, envelopes / LFO, S&H/random... instead, into 0V/10V or -5V/+5V ranges, depending the waveform (even in audio range domain when used as slave clocking module). All LED (inputs, outputs, RUN/STOP button, SET/Cancel buttons) are RGB. Color convention is mainly red for out-of-range input voltages, disabled output, BPM SYNC operation... light blue for regular pulses and gates, green for valid CV and outputs, yellow on "CLK" for BPM CV (Impromptu Modular CLOCKED compatible)...
