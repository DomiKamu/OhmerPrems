...
- COMING SOON (*TLTR* I'm sorry in advance ;): **Entirely free KlokSpid MkII**, the successor of *KlokSpid* (2017 / [Ohmer plugin](https://github.com/DomiKamu/Ohmer), is a 16HP modern "CPU-controlled" and versatile clocking module, it offers 8 independent analog/digital outputs (right side of the module). By default, KlokSpid MkII embeds 6 configurable CV sources (all inputs are located on left side), but can be increased upto 8 by changing "CLK" and "RUN" input roles (both may become bipolar -5V/+5V / unipolar 0/+10V CV input) - either from *Select Input* main display menu, or via context (right mouse click) menu. "CLK" input can be assigned as "CV1" input, instead. "RUN" input can be assigned as "CV2" input. Also, native "CV3" input (default bipolar -5V/+5V CV input, or unipolar 0/+10V CV input) can be assigned as "RESET" input (useful for internal phases reset purposes). Basically, KlokSpid MkII is able to output pulses/gates (min. 1ms duration pulse is default setting, *P-Width* parameter can be set manually, and possibly modulated by any CV sources you want - upto 8), triangles (including ramps/sawtooth up and down, by setting & modulating *Tilt* parameter), sine (first half sine is always pure, unmodifiable, but second half sine can be amplitude-deformed to become *redressed sine*, hump, and any intermediate shape you want), S&H (sample and hold) - using internal random samples generator, variable *Density* (default minimum density is 1 sample per "BPM x ratio" tick, upto 64 samples for 100% density), and 8 customizable wavetables (manually *Morph*able, possibly modulated by any CV sources) by this way, KlokSpid MkII is able to act as envelope generator (by using fitting wavetable). Except for S&H waveforms, each output can be *Phase*-shifted (earl or late), quantized, and played/muted (every "BPM x ratio" tick) via euclidean settings. As master clock, BPM range can be set from 1 upto... 900 (15Hz). As slave clock (when "CLK" is patched to an external clock source, and "CLK" input is set as "External CLK" input - as default setting), the maximum supported frequency is 500Hz (above BPM 900, the external clock speed is indicated as **frequency, in Hz** instead, blue color, or red color while the frequency is internally limited to 500Hz, whilee the source clock is over 500Hz. All LEDs (inputs, outputs, and over RUN/STOP transport button) are RGB - colorscheme is following this convention: RED for out-of-range CV input voltage, disabled output (aka output's modifier is set to "OFF" - useful for CPU/resource saving for unused outputs), also during tempo "SYNC" operation (RUN/STOP LED is red while syncing on external clock source)... CYAN for pulse-based / gate-based input ("CLK" ticks, "RUN" as held gate or run/stop pulse toggle, and possibly "CV3" if assigned as "RESET" pulse-based input, to reset/realign all phases while clock is running), RUN/STOP LED while transport is controlled by pulses/gates (such manual button, sequencer, sequenced switches, and so on), and applied utility voltage to output (these utility modifiers are "GATE", "START", "STOP", and "OFF")... GREEN for RUN/STOP LED while transport is manually controlled by RUN/STOP button on the module, gradual GREEN for valid CV voltage applied on input, and voltage sent to each output (even when the clock is stopped, in case of *V-Offset* setting is not 0V)... YELLOW on "CLK" input, only when slave and set as "BPM CV" (fully compatible with [Impromptu Modular CLOCKED](https://library.vcvrack.com/ImpromptuModular/Clocked) **"BPM CV"** feature)... For each output, most defined parameters can be CV-modulated **by single CV or by multiple combined CVs (upto 8)**: modifier ratio (picked from built-in table, from /16384 to x128, selectable by one CV source only), *Pulse width*/*Tilt*/*Deform*/*Density*/*Morph* (who morph the waveform), voltage amplitude (*Amplitude*), voltage offset (*V-Offset*), pre-offset semitones prior quantization (from *Quantize* menu), *Phase*-shift, and *Euclidean* settings (steps, pulses, offset, and pads). Only base waveform (shape selected from *Waveform* from output-related menu) and *Bipolar around offset* option (from both *Amplitude* and *V-Offset* on-screen settings) cannot be altered by CV. Any output settings (all settings) can be copied to another output. Possibility to reset settings to defaults (factory), for any output (others are not affected). Via a specific option, from context menu ("MISCELLANEOUS" section), you can decide the output 8 generates the module's master phase - **however, when enabled, this option takes priority over output 8 regular settings** - as unipolar 0V/+10V sawtooth signal who follows the module's tempo. Basic selectable waveforms are - from first page of 3, such **PULSE** (default min. is 1ms trigger, 50% for square, 100% for continuous voltages without retrigger), **TRIANGLE** (default 0% is ramp/sawtooth down, 50% generates perfect isosceles triangles, 100% for ramp/sawtooth up, and obviously, any intermediate tilted triangle shape), **SINE** (default 0% is pure sine, but second half sine can be deformed via defined & CV-mod *Deform* setting), and **S&H** (default *Density* is minimum, who generate 1 sample per "BPM x ratio" tick, max. 100% who generate 64 samples per "BPM x ratio" tick). About waveform selection, pages 2 and 3 permit to select a custom wavetable (WT1 to WT4 from page 2/3, WT5 to WT8 from page 3/3), no morphed (aka first WT frame by default).
...
Please notice the module's main display above shows basic waveform selection for current selected output (here's output 1 as current output, as indicated in top-left box). All icons below "PULSE", "TRIANGLE", "SIN", and "S&H" blue labels (into clickable boxes in order to select a waveform for current output), are **animated** (by showing what the *P-width*/*Tilt*/*Deform*/*Density*/*Morph* parameter can do on relevant waveform).

Below, another example by using S&H (sample and hold) waveform, sent to output 1 - output ratio is set to BPM **x1** (set from *Modifier* menu, visible here as first menu item), *Density* (who replaces *P-Width* as third menu item, due to S&H waveform context) is set by unipolar 0V/+10V "CV5" input, voltage amplitude is set to 5V (controlled by unipolar 0V/+10V "CV6" input), bipolar enabled (in this case, min. and max. voltages are spread around *V-Offset*, instead of above offset when bipolar is disabled), voltage offset is set to +2V (controlled by bipolar -5V/+5V "CV7" input). By this way, sample voltages stay inside -0.5V/+4.5V range (5V amplitude around +2V offset). **Phase shift (controlled by "CV8" input) is ignored for S&H waveforms** (in this output-related menu - output 1, you can see the *Phase* menu item is grayed, meaning it can't be selected).

![](doc/images/KlokSpidMkII_Model1_SH.png)

Below, another example of **25% tilted triangle** waveform, sent to output 2 - output ratio is set to BPM **/2** (visible in top left box). *Tilt* parameter (third menu item replaces *P-With* and *Density* seen above, due to triangle waveform context) is set by unipolar 0V/10V "CV5" input (tilt parameter is user-defined to default 0%, here is a constant +0.25V applied on "CV5" input, who set the traingle's *Tilt* parameter to 25%, instead), the shape is visible on VCV Scope module. For triangle waveform, by setting tilt parameter to 0% (either defined by user, and possibily modulated by one or many CV sources), this will output a ramp/saw down signal (right triangle shape). By setting tilt parameter to 100%, this will generate a ramp/saw up signal (right triangle shape, too). By setting tilt parameter to 50%, this will generate a perfect isosceles triangle. In this example, *Amplitude* is set to (default) 10V, *V-Offset* is set to (default) 0V, "bipolar around offset" enabled: voltages generated by this triangle waveform always stay inside -5V/+5V range.
