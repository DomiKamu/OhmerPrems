...
- COMING SOON: **Entirely free KlokSpid MkII**, the successor of *KlokSpid* (2017 / Ohmer Modules), is a 16HP "CPU-controlled" clocking module, 8 analog/digital outputs. It embeds 6 configurable CV sources (upto 8 by changing "CLK" and "RUN" input roles, may become bipolar -5V/+5V or unipolar 0/+10V CV source - "CLK" input can be assigned as "CV1" input, "RUN" input as "CV2" input, and native "CV3" as "RESET" input - useful for internal phase reset purposes). Basically, KlokSpid MkII is able to output pulses/gates (min. 1ms duration, *Pulse width* can be set as fixed, and possibly modulated around fixed setting by any CV source you want), triangles (including ramps up/down, by changing+modulating *Tilt*), sine (first half sine is always fixed, second half can be amplitude-morphed to become redressed sine and hump), S&H (using internal random generator, variable *Density* - min. is one sample per "BPM x ratio" tick), and 8 customizable wavetables (manually *Morph*able, possibly by CV) by this way, KlokSpid MkII can act as envelope generator! Each output can be *Phase*-shifted (except S&H), quantized, and played or not (every BPM x ratio tick) via euclidean rhythm settings. As master clock, BPM range is 1 upto 900 (15Hz). As slave clock (when "CLK" is patched to an external clock source), the max. frequency is 500Hz (above BPM 900, the external clock speed is indicated as **frequency in Hz** as cyan color, or as red color if the frequency is internally limited to 500Hz). All LEDs (inputs, outputs, and RUN/STOP button) are RGB - their color follow this convention: RED for out-of-range CV input voltage, disabled output (aka output's modifier is set to OFF - useful for CPU save if unused outputs), and during tempo "SYNC" operation (RUN/STOP LED, regardling external clock source speed)... CYAN for pulse-based input ("CLK" tick, "RUN" as held gate or RUN/STOP pulse toggle, and possibly CV3 if assigned as "RESET" pulse-based input), and utility voltage applied to output (may be either GATE, START, or STOP)... Variable GREEN for valid CV voltage applied on input, voltage sent to output (even when clock is stopped, if voltage *offset* is not 0V)... YELLOW on "CLK" input - slave & when set as "BPM CV" only (fully compatible with [Impromptu Modular CLOCKED](https://library.vcvrack.com/ImpromptuModular/Clocked) **"BPM CV"** feature)... For each output, most defined parameters (modifier ratio picked from built-in table from /16384 to x128, *Pulse width*/*Tilt*/*Deform*/*Density*/*Morph*, voltage amplitude, voltage offset, quantization offset, and euclidean steps/pulses/offset/fills) can be modulated by CV (only waveform "shape", and phase shift settings can't be modulated by CV). Any output can be copied to another (all settings are, of course, duplicated). Possibility to reset settings to defaults (factory) for any output. Via a specific option from context menu, output 8 is able to output the module's "master" phase - **this option, when enabled, takes priority over output 8 regular settings** - as **unipolar 0V/+10V sawtooth** waveform, this feature may be useful to sync other modules on KlokSpid MkII tempo, for debugging purposes, and so on... TLTR sorry ;)
