...
- 64 steps (max.) per track. LAST STEP (by default, common to all tracks) is manually adjustable via dedicated encoder, and can be shifted (offset - / +) in real time via CV input jack, from minimum 1-step upto 64-step. Every instrument track(s) may have a different LAST STEP, by setting these tracks as SOLO first (indepent Last Step settings are kept later, even if related track(s) is/are unSOLO). AC track's LAST STEP is always defined by module's logic!

- Main display is "touchscreen"-like, mainly for note events editing (silences, notes, AC'd notes, or 8-preset defined ratchets), into 16x64 grid.
...
- PENDULum pattern playback: playing forward from step 1 to track's last step (left to right), then reverse back until step 1, then forward again... like a pendulum or ping-pong. Via applied & held gate-voltage on PENDUL. input jack (+2V min.). 
...
- Ratcheting slots (8, for current pattern) can be edited from FroeZe-X expander (from its **RATCHET** menu, then **EDITOR**).
...
- Polyrhythm: not natively (due to "4/4" nature), but possible by using euclidean rhythm via FroeZe-X expander (TRACK / EUCLID. feature), along independent LAST STEP (set while track(s) is/are SOLO).
...
- In case of main outputs send audio, FroeZe-X expander sends +10V gates (instead of 1ms triggers) on their related output jacks. Each gate is held while related drum kit sample is playing. This may be useful to control an external module, such envelope generator, VCA, filter, dynamics, reverb, delay, another synth...

- Pattern change can be controlled by triggers (previous, next) over "PREV" and "NEXT" input jacks (both must be connected), or by absolute pattern number via 0V/+10V control voltage range, by using "PREV" input only ("NEXT" jack must be disconnected in this case).
