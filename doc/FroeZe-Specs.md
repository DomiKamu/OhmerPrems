FROEZE & FROEZE-X MODULES SPECIFICATIONS
...
- Track naming follows Roland-style (for most of their drum machines): BD for bass drum/kick, SD for snare drum, and so on). Of course, except while audio engine is enabled, track usage is absolutely free! **Big thanks to @k-chaffin** for his suggestion about track naming! ;)
...
- FroeZe sequencer can deliver +10V 1ms triggers, (-10V 1ms in certain situations), or audio (thanks to internal QuadPercs engines).
...
- Any instrument track can be SOLO (except AC track), multiple SOLO is possible.
- Any track can be MUTEd (including AC track), multiple MUTEd tracks is possible.
- MUTE and SOLO can be used at the same time on an instrument track.
- 128 patterns, each of them can be named, up to input box limit, or 128 max. (US/ASCII printable characters, QWERTY/US input only at the moment).
...
- **Under consideration**: SONG mode (incl. integrated song editor) -- delayed...
...
- Polyrhythm: not natively (due to 4/4), but possible via euclidean from FroeZe-X expander (TRACK / EUCLID. page).
...
Free version (without license key) is working as **full player** concerning **both trigger and audio engine QuadPercs modes** (all patterns/tracks are fully playable, even internal QuadPercs engine sounds, in order to keep the highest level of compatibility agains shared patches created/edited by any *OhmerPrems member*). However, without license key and by using any patch made by OhmerPrems member, all controls & edit are locked (every pattern/track/step) - except some saving functions such patterns (any), QuadPercs-engine drum kits, and "8-slot" ratcheting settings (once per pattern).

When not using a configured FroeZe (made by OhmerPrems member) from loaded .vcv patch, after **Initialize** command (**Ctrl+I**, or **Command+I** on Mac), or on new module instance, free version allows to edit **BD**, **SD** and **AC** tracks only, into the pattern #1 (all other patterns are edit-locked). Also, some edit features from FroeZe-X expander are inoperative.

Free version: by using internal (QuadPercs) sounds (except in free player), allowed drum machines are strictly identical than standalone QuadPercs module: Oberheim DMX, Drumulator, LinnDrum, Roland TR-707, and Roland TR-808 drum machines are allowed, at 44.1kHz only (other drum machines and higher sample rates are reserved exclusively to OhmerPrems members). However, any drum machine (and higher sample rate than 44.1kHz) can be selected, but their sounds are muted (without any notification).
