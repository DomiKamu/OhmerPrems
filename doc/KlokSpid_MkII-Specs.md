...
- "CPU-controlled"-like versatif clock generator (standalone), or clock modulator (slave mode, when CLK input is patched to another clocking source), including a main touchscreen-like display, plus an info display below the continuous encoder, usable as low-frequency oscillator (LFO), optional (per-output) euclidean rhythms, and optional pitch quantizer (may be active, even when the clock is stopped).
...
- Output waveform can be either PULSE, TRIANGLE (including ramp down, and ramp up, via _Tilt_ parameter), SINE (including half sine, and humps), and Sample & Hold (S&H) on first page (of 3). Pages 2 and 3 are dedicated to (optional) custom wavetables, from WAVETABLE 1 to WAVETABLE 8. At the moment, only 32-bit float IEEE WAVE (.wav) **Xfer Serum-compliant** wavetable files will be supported (wavetable support remains under development, not yet operational).

- The module can be controlled either by continuous encoder + **SET** (blue) button + **Cancel** button _"combo"_, and/or by touchscreen, at you convenience. However, some controls (like page arrows) are usable by touchscreen only. In general, the continuous encoder selects next or previous selectable item, such menu item, box, checkbox, button, piano key - **selected item is always highlighted by yellow color, you can't miss it** - also can be used to increase/decrease a parameter value (by moving the horizontal slider). **SET** button (the blue button seating over the continuous encoder) permits to validate the selected menu/box item, check/uncheck a checkbox, to activate the parameter slider (entering in edit mode) or entering deeply in menu system. **Cancel/Exit** button (who have a red cross symbol around) leaves a parameter edit, return to previous menu, cancel BPM edit, cancel slave mode edit... Please notice both input and output selection screens (have eight clickable boxes) don't have a touchscreen control to back, so use the **Cancel/Exit** button instead. Also, most parameters are validated, even when you click Cancel/Exit button (or top-left box icon on most pages).

- On displays, colorscheme is: cyan = selectable item. Yellow = current selection/active menu item/box, editing BPM, editing slave mode... Gray = disabled item (locked).
