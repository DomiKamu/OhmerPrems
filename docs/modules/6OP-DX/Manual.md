...
This _6OP-DX_ module is mainly useful to prepare your DX7 synthesizer file(s) you'll need in your project (can be single-voice file, 32-voice cartridge file, or full synthesizer file). When your sound design session is completed (and of course, saved), you'll can replace the huge module by its compact variant, the _6OP-DX Kompakt_, in order to save space in your rack. Of course, you'll must open appropriate file(s), either **.6opsynth** file (single-voice), **.6opcart** file (32-voice cartridge) file, or **.6opsynth** file (full synthesizer), previously made by any _6OP-DX_ module. Please read [6OP-DX Kompakt User's Manual](https://github.com/DomiKamu/OhmerPrems/blob/v2/docs/modules/6OP-DX/Manual.md) for more details about this thinner module!

Also, you'll can prepare your custom DX7 soundbank, then export it as _VMEM_ SysEx (32-voice soundbank) or _VCED_ SysEx (single-voice), in order to import it in real DX7 synthesizer (the DX7 always uses MIDI channel 1 for SysEx operations, also, MEMORY PROTECT must be disabled prior to import), or from another DX7-compatible software capable to import SysEx files, like freeware [**Dexed**](https://asb2m10.github.io/dexed/), or commercial [**Arturia's DX7 V**](https://www.arturia.com/products/software-instruments/dx7-v/overview) / [**Plogue's chipsynth OPS7**](https://www.plogue.com/products/chipsynth-ops7.html), and possibly other third-party softwares, such SysEx Librarian (MacOS X computers), MIDI-OX (Windows computers), and so on.
