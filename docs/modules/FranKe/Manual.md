...
When external clock is set to **BY PULSES (32 PPQN)** (it's the default setting), or **BY PULSES (24 PPQN)**, and patched, the LED turns on, cyan. The CLOCK input jack needs to receive **at least two consecutive triggers** in order to establish the frequency (Hz), and by this way, the global tempo. 32 PPQN resolution was choosen as default to be coherent with _FroeZe_ sequencer module, who are using the same resoluion. 24 PPQN is useful when VCV Rack (Pro) is used as plugin from DAW, because the VCV DAW module provides 24 PPQN as maximum clock resolution. By pulses (either 32 or 24 PPQN), maximum supported frequency by the module is **16Hz (960 BPM)**, because they're too many signifiant clocking unstabilities above this frequency. It's highly recommended to use triggers/pulses (or gates) for the external clock module (avoid sine/triangle/saw/ramp/noise/S&H waveforms provided by a LFO module!)
...
As soon as any frequency is established, this new frequency is registered in the module's memory as "last known" frequency. This registered frequency will be used for standalone clock  in case of the CLOCK input jack is disconnected (later).
