...
Each track (line) have its own **PITCH**, +10V **GATE**, and (optional to use) **VELocity** (or +10V 1ms **SOC** trigger) output jacks, on the same row. Please consider **VEL**ocities are applicable only by melodic (notes) tracks, for other roles, a +10V 1ms trigger is send as **SOC** ("Start of Cycle") when the related track restart (repeat) its sequence.
...
:warning: _FranKe_ module still under development, it will be available "soon" (planned to public release around mid-April, or third week of April, 2026).
