...
In case you're loading a shared patch (.vcv file) made by OhmerPrems member (who are using full version for him/her), and if the patch file embeds one (or many) _FranKe_ module instance(s), all of these instances are working as (full) **player**, this is meaning these modules are able to playback without any restriction (as expected for "a player"). However, **everything** is locked against editing, including MUTE and SOLO states (individual/global), CV assignment (from module's SETUP), reverse and pendumum play by momentary button (who affect individual track), track role, randomizations, module reset (via **Initialize** command from module's contextual menu, or **Ctrl**+**I** / **Cmd**+**I** on MacOS X, as keyboard shortcut), pattern/track copy/paste operation, and any file operation.

In case you're adding - by yourself - a new instance of _FranKe_ module in the rack, from the fresh module, patterns from 03 to 64 can't be selected (either by potentiometer, or by voltage), from patterns 01/02, **only tracks 1 and 2 can be edited**, all other tracks (from 3 to 8) are locked against edition, including "Randomize" (from module's contextual menu ** Randomize** command, or **Ctrl**+**R** / **Cmd**+**R** on MacOS X, as keyboard shortcut), CV1 and CV2 may be assigned, but their actions may be limited, or restricted. Turing Machine sequence on tracks 1 and/or 2 can be edited and exported (imported lated to track 1 or 2). Track role (via **TRK. ROLE** momentary button) can be changed for tracks 1 and 2 only. Also, many contextual menu entries concerning track, pattern and file operations may be disabled (grayed), or hidden, depending the module's context when the contextual menu is pulled down (on right-mouse button click, or by using equivalent keyboard shortcut). All others still locked/disabled, until you'll become OhmerPrems member!
