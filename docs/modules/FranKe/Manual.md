...
_FranKe_ (a tribute to [Christopher Franke](https://en.wikipedia.org/wiki/Christopher_Franke), member of Tangerine Dream band during best-known lineup during mid-1970s trio with Edgar Froese and Peter Baumann) - is providing 64 patterns, 8 tracks per pattern, 16 steps per track.

Track may be melodic (by using notes, from C-1 to G9, with possible flat or sharp accidentals), or free CV voltages (named CV OUT) who can send any voltage inside -10V to +10V (0.01V resolution).
...
This is a French-spoken 35min. [video](https://www.youtube.com/watch?v=BAPuK8xuvAo&pp=ygUNb2htZXIgbW9kdWxlcw%3D%3D) (the video have embedded English closed captions), from [Ohmer Modules channel](https://www.youtube.com/@OhmerModulesVCVRack) on YouTube.
...
:warning: FranKe module is under development, it will be available "soon" (planned to public around mid-Aprilor third weed of April, 2026.

----

Free version (without license V2 keyfile) is working as **full player** (it can play any patch made by OhmerPrems member, without any restrictions). However, without a valid V2 license keyfile, **only track 1 and the Turing Machine can be edited, pattern 1 only** (like FroeZe module), other are locked against edition (including "Randomize" from module's contextual menu). All stuff made on _Franke_ module is always saved and recalled.
