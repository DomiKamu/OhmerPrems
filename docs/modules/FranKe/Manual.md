...
While the sequencer is running, all tracks (including unused roles) are processed, despite 8 roles are active simultaneously, a total of 24 roles (like 24 tracks) are sequenced at the same time (8 tracks x 3 roles), and depending your computer, this may impact the CPU usage.
...
:warning: Please keep in mind **FranKe module is not an instrument**, but a voltage-based (analog) step-sequencer, who sends pitch voltages ("V/Oct" compliant) or free voltages, 0V/+10V gates, and possibly velocities voltages, in order to control other modules, such VCO, synth voice, enveloppe generator, VCA, VCF cutoff/resonance/drive, and... any module you'll want to control via sequenced velocities. Please consult OUTPUT JACKS section, below, for more details.

Like future (still in development) [6OP-DX synth voice module](../6OP-DX/Manual.md), FranKe provides 8 different models (aka GUI theme variations), shown as animation on the top of this user's manual. Model is compliant against **Prefer dark panels if available** VCV Rack's setting (from **View** menu) as proposal from module's browser. Existing models are **Aluminium**, (it's the default model if _Prefer dark panels if available_ setting is disabled/unchecked), **Stage Repro** (red theme), **Cobalt** (blue theme), **Absolute Night** (it's the default model if _Prefer dark panels if available_ setting is enabled/checked), **Dark "Signature"**, **Fort Knox "Signature"**, **Oxide "Signature"**, and **Titanium "Signature"**.
...
Step-recording is a possible way to define notes (semitones) for either melodic and modulation (CV OUT) tracks. For melodic tracks, recorded steps are always notes/semitones by names (like F#3, Ab4, C5, etc). For modulation tracks, the notes names/alterations/octaves are simply replaced by their respective voltages.

To engage step-recording, press STEP-REC button located in left-side momentary buttons (this button is the 2nd from top, near CLOCK input jack).

To finish step-recording session, press EXIT button (this button is the same than used to access module's SETUP). Also by selecting a Turing Machine track, or by starting the transport (via voltage on RUN input jack) will disengage the step-recording automatically!
...
The velocity cable is useless by these situations:

- You are using only left-side "piano" buttons to set a note (or voltage), or/and editing a note/voltage by turning the related step encoder.
- You are recording only over a modulation (CV OUT) tracks.
