...
Like real Prophet VS synthesizer, the _Vektor_ module is using four **digitally-generated oscillators**, every is a sample-based waveform (except for white noise, generated by hybrid sampling/synthesis before buffered). It's the reason why the _Vektor_ module is assumed as "quad" and uses it as tag for module search criteria (from VCV Rack 2 [module browser](#outro)).
...
To select next (or previous) waveform, rotate the **DATA ENTRY** continous encoder clockwise (or counter-clockwise). The selection cycles to **000** (or **127**) as required, when an _edge_ of the list is reached.
