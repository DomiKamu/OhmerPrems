...
Also provided by _Vektor_, two internal (independent) low-frequency oscillators (**LFO 1** and **LFO 2**), plus **FM input** jack, able to work as either **TZ FM** (linear Through-Zero FM) or **PM** (Phase Modulation, variant of FM synthesis used by Yamaha DX synthesizers). These possible **frequency modulators** are designed to modulate any oscillator (waveform) you'll want (OSC A, B, C, or/and D), offering near infinite possibilities in sound design. Also, LFO 1 and/or LFO 2 can be used by external module(s) in your rack, by attaching the _VX_ expander alsongside _Vektor_ (right-side, without space between them!), for enabled low-frequency oscillator(s).

However, many parts of the real Prophet VS synthesizer, such analog low-pass filter, ADSR envelope generators, modulation matrix, stereo and panning, and stereo chorus, aren't provided by the _Vektor_ module, assuming they're a lot of capable third-party modules to do similar tasks inside our virtual Eurorack modular environment! By this way, when used "alone", _Vektor_ cannot be considered as "ready-to-use" synth voice module, like most VCO modules.

_Vektor_ is using four independent waveform-based oscillators, named **OSC A**, **OSC B**, **OSC C** and **OSC D** (or by simple letters **A**, **B**, **C**, and **D** also refer to respective OSC A, OSC B, OSC C, and OSC D), located in four angles of the diamond (as described by previous topic). All of them are mixed (crossfading) either manually by the joystick (or by external voltages applied to **X** and **Y** input jacks), or via fully automated **MIX ENV**elope (controlled by **GATE** input jack).
...
- Underscore (_)
- Equals sign (=)
- Period (.) - **please avoid it as leading character**, because Linux and MacOS X operating systems consider these files as hidden!
...
Even the **MIX** output jack is considered as the most important in Vector Synthesis universe, because all oscillators are constantly mixed (manually by the joystick **or** by applied voltages on both **X** and **Y** input jacks, and possibly by the MIX ENVelope).

As first benefit, by using discrete A, B, C, and D output jacks only (without **MIX**), in this case you get **four independent oscillators** (or sound sources), whatever the Vector Synthesis setting of the _Vektor_ module. 

As second benefit, you are able to apply particular FX processing on discrete A, B, C, and/or D output signal(s), like delays or reverbs, and use it/them together with the **MIX** output jack. All of these outputs can be used at the same time, either as final audio output, or as possible modulation source for another module.
...
As soon as a _Vektor_ module is installed in your rack (it's a _module instance_, or _instantiated module_), it does a quick self-test sequence. You'll can freely change its model anytime you'll want: from right click context menu, select **Model** menu item, then select the model you'd like from the submenu (Aluminium, Stage Repro, and so on).
