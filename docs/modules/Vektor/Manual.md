...
The best way to present the _Vektor_ module layout is by the following **5 minutes** animation (22 frames, 15s/frame):

![](_img/VektorLayoutV4.gif)
...
Like real Prophet VS synthesizer, the _Vektor_ module is using four **digitally-generated oscillators**, every is a sample-based waveform (except for white noise, generated by hybrid sampling/synthesis before buffered). It's the reason why the _Vektor_ module is assumed as "quad" and uses it as tag for module search criteria (from VCV Rack 2 [module browser](#modulebrowser)).

Each oscillator is identified by a letter: **A**, **B**, **C**, and **D**. In the 2D environment of _Vector Synthesis_ (into the "diamond"), the convention is oscillator A at left-hand, oscillator B at top, oscillator C at right, and oscillator D at bottom:
