...
An oscillator is, obviously, the most important part of a VCO module, because it's the sound source, exactly like a drummer or bassist in a band!

Like real Prophet VS synthesizer, the _Vektor_ module is using four **digitally-generated oscillators**, every is sample-based waveform (exception for white noise, who is generated by hybrid sampling/synthesis technique). It's the reason why the _Vektor_ module is assumed as "quad" and uses it as tag for module search (from [module browser](#outro)).
